...
fn main() -> Result<()> {
    ...
    match &cli.command {
        Commands::Bundle { entry, output } => {
            ...
            // エントリーポイントが存在するか確認
            if !entry.exists() {
                return Err(anyhow!("エントリーポイントファイルが存在しません: {}", entry.display()));
            }
...
/// Nixファイルをバンドルする関数
fn bundle_nix_files(entry_point: &Path) -> Result<String> {
    // 処理済みファイルを追跡するためのセット
...
fn validate_with_nix_instantiate(output_file: &Path) -> Result<()> {
